...
if grep -q '"feishu"' "$CONFIG_FILE" 2>/dev/null; then
    ...
    info "飞书配置指南: https://github.com/wanikua/become-ceo/blob/main/飞书配置指南.md"
fi
...
" 2>/dev/null)
    ...
    if [ -n "$MISSING_IDENTITY" ]; then
        ...
        info "参考模板补全: https://github.com/wanikua/become-ceo/blob/main/openclaw.example.json"
        echo ""
        echo -e "${CYAN}  📋 修复方法：${NC}"
