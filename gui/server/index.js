...
const app = express();
const PORT = process.env.CEO_GUI_PORT || 18795;

// SEC-03: 不再使用硬编码默认 Token
import crypto from 'crypto';
let AUTH_TOKEN = process.env.CEO_AUTH_TOKEN;
if (!AUTH_TOKEN || AUTH_TOKEN === 'changeme') {
  AUTH_TOKEN = crypto.randomBytes(16).toString('hex');
  console.warn('');
  console.warn('╔══════════════════════════════════════════════════════════════╗');
  console.warn('║  ⚠️  安全警告: CEO_AUTH_TOKEN 未设置或使用了默认值!       ║');
  console.warn('║  已自动生成随机 Token（仅本次运行有效）                     ║');
  console.warn('║  请设置环境变量: export CEO_AUTH_TOKEN=$(openssl rand -hex 16) ║');
  console.warn('╚══════════════════════════════════════════════════════════════╝');
...
app.get('/api/weather', authMiddleware, async (req, res) => {
  ...
  try {
    const weatherResp = await fetch(`https://wttr.in/${encodeURIComponent(location)}?format=j1`, {
      ...
      headers: { 'User-Agent': 'ceo-gui/1.0' }
    });
...
const BIND_HOST = process.env.CEO_BIND_HOST || (IS_DOCKER ? '0.0.0.0' : '127.0.0.1');
